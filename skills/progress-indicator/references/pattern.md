...
**Source:** apps/web/content/patterns/user-feedback/progress-indicator.mdx
...
- File upload and export jobs where users need a clear, repeatable interface model.
- Multi-step setup or checkout flows where users need a clear, repeatable interface model.
- Background tasks with measurable completion where users need a clear, repeatable interface model.

## Benefits

- Clarifies how progress indicator should behave before implementation details begin to sprawl.
- Creates a reusable interaction model for teams who need to show how far a task has advanced and how much work remains when the system can estimate that honestly.
...
```mermaid
flowchart TB
Root[Progress Indicator] --> A[Task label]
Root --> B[Progress track]
Root --> C[Current value]
Root --> D[Supplementary detail]
Root --> E[Completion state]
```
...
- Explains what is currently progressing.

2. **Progress track**

- Visualizes the full range of the task.

3. **Current value**

- Shows the current amount or step that has completed.

4. **Supplementary detail**

- Adds percent, count, or time remaining context.

5. **Completion state**

- Transitions into success or next-step guidance cleanly.

#### Summary of Components

| Component | Required? | Purpose |
| --- | --- | --- |
| Task label | ✅ Yes | Explains what is currently progressing. |
| Progress track | ✅ Yes | Visualizes the full range of the task. |
| Current value | ✅ Yes | Shows the current amount or step that has completed. |
| Supplementary detail | ❌ No | Adds percent, count, or time remaining context. |
| Completion state | ❌ No | Transitions into success or next-step guidance cleanly. |

## Variations

### Linear progress bar

Uses a horizontal track and fill.

**When to use:** Use for uploads, exports, and determinate waits.

### Circular progress

Uses compact radial feedback.

**When to use:** Use when space is tight or the number needs emphasis.

### Step progress

Shows a discrete sequence of milestones.

**When to use:** Use for multi-step flows and onboarding.
...
- A clear baseline implementation of progress indicator that can be reviewed without framework-specific noise.
...
- [ ] Verify the default, loading, error, and success states for progress indicator.
...
- [`@radix-ui/react-progress`](https://www.npmjs.com/package/%40radix-ui%2Freact-progress) - Accessible progress primitive for determinate and indeterminate states.

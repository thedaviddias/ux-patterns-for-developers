...
const SubscribeForm = dynamic(() =>
	import("@app/_components/subscribe").then((m) => ({
		default: m.SubscribeForm,
	})),
);
...
export async function generateMetadata({ params }): Promise<Metadata> {
	...
	const pathSegments = Array.isArray(resolvedParams.mdxPath)
	...
	const isHomepage =
		!resolvedParams.mdxPath || resolvedParams.mdxPath.length === 0;
...
export default async function Page(props: PageProps) {
	...
	const isPatternPage =
		pathSegments[0] === "patterns" && pathSegments.length > 2;
	const isPatternCategory =
		pathSegments[0] === "patterns" && pathSegments.length === 2;
	const isPatternsIndex =
		pathSegments[0] === "patterns" && pathSegments.length === 1;
	const isGettingStarted =
		pathSegments.join("/") === "patterns/getting-started";
	...
	if (isHomepage) {
		...
		const datePublished =
			extendedMetadata?.date || extendedMetadata?.datePublished;
		const dateModified =
			extendedMetadata?.lastModified ||
		...
		if (items.length > 0) {
			...
			if (validItems.length > 0) {
				schemas.push(
					generateItemListSchema(title, description, path, validItems),
				);
			}
		}
	...
	return (
		<>
			{schemas.map((schema) => {
				const schemaType =
					(schema as { "@type"?: string })["@type"] || "Schema";

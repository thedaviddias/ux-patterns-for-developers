...
export default async function PageRoute(props: {
	...
	// Only handle specific pages that we know exist in the pages directory
	const allowedPages = ["privacy-policy", "about"];
	if (!allowedPages.includes(params.pageName)) {
		notFound();
	}

	// Get the page from the pages directory
	const page = source.getPage(["pages", params.pageName]);
	if (!page) {
		notFound();
	}
	...
	const title =
		pageData.title ||
		params.pageName.charAt(0).toUpperCase() +
			params.pageName.slice(1).replace(/-/g, " ");
	const description = pageData.description || `Information about ${title}`;
	...
	const schemas = [
		...
		generateBreadcrumbSchema([
			...
			{ name: title, url: `/pages/${params.pageName}` },
		]),
	];
...
export async function generateStaticParams() {
	// Only generate params for allowed pages
	const allowedPages = ["privacy-policy", "about"];

	return allowedPages.map((pageName) => ({
		pageName,
	}));
}
...
export async function generateMetadata(props: {
	...
	const title =
		...
		params.pageName.charAt(0).toUpperCase() +
			params.pageName.slice(1).replace(/-/g, " ");
	const description = page.data.description || `Information about ${title}`;
	...
	return {
		...
		openGraph: {
			...
			title,
			description,
			url: `https://uxpatterns.dev/pages/${params.pageName}`,
			images: [
				{

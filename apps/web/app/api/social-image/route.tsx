...
async function loadGoogleFont(
	...
	const resource = css.match(
		/src:\s*url\(([^)]+)\)\s*format\(['"](woff2?|opentype|truetype)['"]\)/,
	);
...
export async function POST(request: NextRequest) {
	try {
		...
		const size =
			PLATFORM_SIZES[platform as keyof typeof PLATFORM_SIZES] ||
			PLATFORM_SIZES.instagram;
		...
		const [poppins300, poppins400, poppins500, poppins700, poppins800] =
			await Promise.all([

...
<BuildEffort
  level="low"
  description="Uses native HTML anchor elements with CSS for hover/focus states. Minimal JavaScript needed. Main effort is in consistent styling and accessibility labeling."
/>
  ...
  alternatives={[
    {
      ...
      when: "the action doesn't navigate to a URL but triggers an in-page behavior",
      pros: ["Clear action semantics", "No URL needed", "Form submission support"],
      cons: ["Not for navigation", "No browser history integration"]
    },
    {
      name: "Breadcrumb",
      path: "/patterns/navigation/breadcrumb",
      when: "showing hierarchical location with navigable path segments",
      pros: ["Shows site structure", "Contextual navigation"],
      cons: ["Only for hierarchy display", "Not for general linking"]
    },
    {
      name: "Navigation Menu",
      path: "/patterns/navigation/navigation-menu",
      when: "organizing a collection of links into structured site navigation",
      pros: ["Organized navigation", "Clear site structure"],
      cons: ["Overhead for single links", "Complex for simple linking needs"]
    }
  ]}
/>
...
<Playground patternType="navigation" pattern="link" example="basic" presentation="hidden-code" />
...
<BrowserSupport features={["html.elements.a", "css.selectors.link", "css.selectors.visited", "css.properties.text-decoration-thickness"]} />
  ...
  items={[
    {
      ...
      answer:
        "A link (anchor element) navigates the user to a new page or resource and has a URL destination. A button triggers an in-page action like submitting a form, opening a modal, or toggling a state. Use <a> for navigation and <button> for actions.",
    },
    {

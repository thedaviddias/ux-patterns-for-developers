...
popularity: high
bestFor:
  - Search results and archives where orientation matters
  - Large datasets that need shareable URLs or recovery
  - Interfaces where users compare items across sessions
avoidWhen:
  - Browsing should feel uninterrupted and feed-like
  - Users rarely need exact page jumps
  - Mobile-first discovery matters more than precise location
compareWith:
...
<BuildEffort
level="medium"
  description="Requires page navigation logic, active state management, keyboard accessibility, and [ARIA attributes](/glossary/aria-attributes) (`aria-current`, `aria-label`)."
/>
<GuidesBanner
  title="Pagination vs Infinite Scroll vs Load More?"
  description="Compare content loading patterns to make the right choice"
...
<PatternComparison
  current="Pagination"
  title="Choose the right loading pattern"
  description="Pagination is strongest when people need orientation, shareable URLs, and predictable recovery. Compare it with the lighter-weight loading patterns before you commit."
  alternatives={[
    {
      name: "Load More",
      path: "/patterns/navigation/load-more",
      when: "users browse in batches and should decide when the next set of results appears",
      pros: ["Balanced default", "Keeps context", "Footer stays reachable"],
      cons: ["No exact page jumps", "Weaker SEO and shareability than pagination"]
    },
    {
      name: "Infinite Scroll",
      path: "/patterns/navigation/infinite-scroll",
      when: "discovery should feel uninterrupted and precise recovery matters less than momentum",
      pros: ["Lowest browsing friction", "Strong mobile feel", "Engagement focused"],
      cons: ["Harder to revisit results", "Footer access and recovery are weaker"]
    }
  ]}
/>
...
items={[
    ...
    },
    {
      ...
      answer:
        "Use unique URLs for each page, implement rel='prev' and rel='next' links, ensure all pages are crawlable, and avoid duplicate content issues with proper [canonical tags](/glossary/canonical-tags).",
    },
    {

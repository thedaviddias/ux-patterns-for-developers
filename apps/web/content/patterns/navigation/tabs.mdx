...
<BuildEffort
level="medium"
  description="Requires proper ARIA roles (tablist, tab, tabpanel), keyboard arrow-key navigation between tabs, controlled active state management, and responsive handling for overflow tabs. Optional features like URL synchronization and [lazy loading](/glossary/lazy-loading) add complexity."
/>
## Use Cases
...
current="Tabs"
  alternatives={[
    ...
    },
    {
      ...
      pros: ["Full [page navigation](/glossary/pagination)", "URL-based routing", "SEO-friendly"],
...
<Playground patternType="navigation" pattern="tabs" example="basic" presentation="hidden-code" />
...
import { Playground } from "@/components/playground";es when tabs overflow
...
items={[
    ...
    },
    {
      ...
      answer:
        "Use Arrow Left and Arrow Right keys to navigate between tabs. The Tab key should move focus from the active tab to the tab panel content. This follows the WAI-ARIA Authoring Practices for tab interfaces and is the expected behavior for [screen reader](/glossary/screen-reader) users.",
    },
    {

...
A well-built navigation menu balances visibility, hierarchy, and responsiveness — always adapting to [viewport](/glossary/viewport) size while remaining accessible to keyboard and [screen reader](/glossary/screen-reader) users.<BuildEffort
  level="medium"
  description="Requires responsive layout handling, active state management, optional dropdown submenus, [keyboard navigation](/glossary/keyboard-navigation), and ARIA landmark labeling. Sticky positioning and mobile collapse add further effort."
/>
## Use Cases
  ...
  alternatives={[
    {
      ...
      when: "the app has deep navigation requiring a persistent vertical structure",
      pros: ["Supports tree navigation", "Always visible", "Good for many items"],
      cons: ["Takes horizontal space", "Not conventional for marketing sites"]
    },
    {
      name: "Tabs",
      path: "/patterns/navigation/tabs",
      when: "switching between parallel content views at the same hierarchy level",
      pros: ["Clear active state", "Quick switching", "Familiar pattern"],
      cons: ["Limited to few items", "Not for site-wide navigation"]
    },
    {
      name: "Breadcrumb",
      path: "/patterns/navigation/breadcrumb",
      when: "showing hierarchical location as a secondary navigation aid",
      pros: ["Shows context", "Lightweight", "Good for deep sites"],
      cons: ["Supplementary only", "Cannot replace primary navigation"]
    }
  ]}
/>
...
<Playground patternType="navigation" pattern="navigation-menu" example="basic" presentation="hidden-code" />
...
import { Playground } from "@/components/playground"; becomes sticky
- **Timing:** 200ms ease
...
<FaqStructuredData
  items={[
    {
      question: "What is a navigation menu?",
      answer:
        "A navigation menu is a collection of links organized into a bar or list that provides access to the main sections of a website or application. It is typically positioned at the top of the page and serves as the primary way users move between pages.",
    },
    {
      question: "How many items should a navigation menu have?",
      answer:
        "Primary navigation menus should contain 5-7 top-level items for optimal scannability. Research shows that users struggle to scan and process more than 7 items quickly. If you have more sections, group them into categories with dropdowns or use a megamenu pattern.",
    },
    {
      question: "Should navigation be sticky or static?",
      answer:
        "Sticky navigation is recommended for long pages or applications where users need constant access to navigation. For short pages or content-focused sites, static navigation at the top is sufficient. Consider using a compact sticky nav that shrinks on scroll to save space.",
    },
    {
      question: "How do I indicate the current page in navigation?",
      answer:
        "Visually distinguish the active item using color, font weight, an underline, or a background highlight. For accessibility, add the aria-current='page' attribute to the active link so screen readers can announce the current page.",
    },
    {
      question: "How do I handle navigation on mobile?",
      answer:
        "Collapse the horizontal navigation into a hamburger menu or off-canvas panel at mobile breakpoints. Ensure the toggle button is clearly visible and accessible. The collapsed navigation should support all the same links and keyboard interactions as the desktop version.",
    },
  ]}
/>

...
<BuildEffort
level="high"
  description="Needs [lazy loading](/glossary/lazy-loading), API handling, memory management, and edge cases like scrolling position retention."
/>
<GuidesBanner
  title="Infinite Scroll vs Pagination vs Load More?"
...
<PatternComparison
  ...
  description="Infinite scroll is best for feed-like discovery. Compare it with the more controlled loading patterns before using it by default."
  alternatives={[
    {
      name: "Pagination",
      path: "/patterns/navigation/pagination",
      when: "users need precise navigation, stable URLs, or an easy way to return to a known spot",
      pros: ["Clear orientation", "SEO friendly", "Footer stays accessible"],
      cons: ["Requires explicit navigation", "Feels less seamless for passive browsing"]
    },
    {
      name: "Load More",
      path: "/patterns/navigation/load-more",
      when: "users should keep control over when more items appear without switching pages",
      pros: ["User control", "Footer stays accessible", "Easier to tame than infinite scroll"],
      cons: ["Adds clicks", "Less momentum for endless discovery"]
    }
  ]}
/>

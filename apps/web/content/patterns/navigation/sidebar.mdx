...
<BuildEffort
  level="medium"
  description="Requires collapsible/expandable state management, nested tree navigation, responsive behavior (overlay on mobile, fixed on desktop), sticky positioning, and active state tracking with route integration."
/>
  ...
  alternatives={[
    {
      ...
      when: "navigation is simple enough for a horizontal top bar",
      pros: ["Familiar pattern", "Saves horizontal space", "Good for marketing sites"],
      cons: ["Limited item count", "No deep hierarchy support"]
    },
    {
      name: "Hamburger Menu",
      path: "/patterns/navigation/hambuger-menu",
      when: "navigation should be hidden by default to maximize content space",
      pros: ["Maximum content space", "Clean interface", "Mobile-first"],
      cons: ["Hidden discoverability", "Extra click required"]
    },
    {
      name: "Tabs",
      path: "/patterns/navigation/tabs",
      when: "switching between parallel views at the same level without hierarchy",
      pros: ["Clear active state", "Quick switching", "No vertical space cost"],
      cons: ["Limited to few items", "Flat hierarchy only"]
    }
  ]}
/>
...
<Playground patternType="navigation" pattern="sidebar" example="basic" presentation="hidden-code" />
...
{hasExpanded && isExpanded && <SectionContent />}
```
...
<FaqStructuredData
  items={[
    {
      question: "What is a sidebar navigation pattern?",
      answer:
        "A sidebar is a persistent vertical navigation panel positioned along the left or right edge of the screen. It provides access to the main sections of an application and supports hierarchical navigation through collapsible sections and nested links.",
    },
    {
      question: "When should I use a sidebar instead of a top navigation bar?",
      answer:
        "Use a sidebar when your application has many navigation items that benefit from vertical space, when you need to display nested hierarchies, or when the app follows a dashboard or admin panel pattern. Top navigation bars work better for marketing sites with fewer items.",
    },
    {
      question: "How do I handle the sidebar on mobile devices?",
      answer:
        "On mobile, convert the sidebar into an overlay that slides in from the edge of the screen with a backdrop. Provide a visible toggle button in the header to open and close it. Support Escape key and tap-on-backdrop to dismiss.",
    },
    {
      question: "What is mini or collapsed sidebar mode?",
      answer:
        "Mini mode collapses the sidebar to a narrow strip showing only icons, expanding the content area. Hovering or focusing on an icon shows a tooltip with the full label. Users can toggle between full and mini modes with a collapse button.",
    },
    {
      question: "How do I persist sidebar state across page navigations?",
      answer:
        "Store the collapse and section expand/collapse states in localStorage, a global state manager, or derive them from the current URL. Auto-expand the section containing the active page on load so users always see their current location.",
    },
  ]}
/>

...
popularity: high
bestFor:
  - Short freeform answers such as names, titles, and labels
  - Structured input with lightweight validation
  - Familiar one-line data entry in forms
avoidWhen:
  - Users need formatting, long-form writing, or rich content
  - The answer should come from a known list of options
  - The field has multiple tightly related values
compareWith:
  - name: "Textarea"
...
<BuildEffort
level="low"
  description="Requires basic HTML input handling, validation, [keyboard navigation](/glossary/keyboard-navigation), and optional accessibility attributes."
/>
<GuidesBanner
  title="Text vs Number vs Email Input?"
  description="Compare input types to make the right choice"
  guidePath="/pattern-guide/choosing-input-types"
/>
  ...
  alternatives={[
    {
      ...
      when: "users select from predefined options while typing",
      pros: ["Reduces errors", "Faster input", "Guided selection"],
      cons: ["More complex", "Requires data source", "Limited flexibility"]
    },
    {
      name: "Date Picker",
      path: "/patterns/forms/date-picker",
      when: "users need to input dates with calendar visualization",
      pros: ["Visual calendar", "Date validation", "No format confusion"],
      cons: ["Only for dates", "Takes more space", "Complex interaction"]
    },
    {
      name: "Multi-Select",
      path: "/patterns/forms/multi-select-input",
      when: "users select multiple items from predefined options",
      pros: ["Clear options", "Multiple selections", "No typing needed"],
      cons: ["Limited to presets", "Takes more space", "Not for freeform"]
    }
  ]}
/>
...
<ChecklistDownload patternSlug="text-field" />

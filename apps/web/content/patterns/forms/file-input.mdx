...
items={[
    {
      ...
      answer:
        "The most reliable approach is to visually hide the native input with `opacity: 0; position: absolute;` (not `display: none`, which breaks keyboard access) and create a custom-styled button or drop zone that programmatically triggers the input via `.click()`. Keep the native input in the [DOM](/glossary/dom) and accessible by keyboard.",
    },
    {

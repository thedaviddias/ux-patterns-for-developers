...
popularity: high
bestFor:
  - Long option lists where typing can narrow the field
  - Inputs where users may know only part of the answer
  - Repeated data entry that benefits from speed
avoidWhen:
  - The option set is short enough for radio buttons or select
  - Matching is unreliable and suggestions will feel noisy
...
compareWith:
  ...
  - name: "Multi-select Input"
    href: "/patterns/forms/multi-select-input"
complexity: medium
...
<BuildEffort
level="medium"
  description="Requires live filtering, real-time suggestion updates, [keyboard navigation](/glossary/keyboard-navigation), and [screen reader](/glossary/screen-reader) compatibility using [ARIA attributes](/glossary/aria-attributes)."
/>
...
<Playground patternType="forms" pattern="autocomplete" example="basic" presentation="hidden-code" />

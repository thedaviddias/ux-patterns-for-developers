...
<BuildEffort
  level="medium"
  description="Requires structured state, keyboard handling, and resilient feedback for search through content efficiently."
/>

<GuidesBanner
  title="Search Field vs Command Palette?"
  description="Compare visible search with a keyboard-first command surface"
...
<PatternComparison
  ...
  title="Choose visible search vs richer alternatives"
  description="Search field is the default when the job is retrieval. Compare it with a keyboard-first launcher, a suggestion-heavy input, and a plain field before treating them as interchangeable."
  alternatives={[
  {
    "name": "Command Palette",
    "path": "/patterns/advanced/command-palette",
    "when": "users need one keyboard-first surface for actions, destinations, recent items, and power-user shortcuts",
    "pros": [
      "Fast for expert users",
      "Unifies actions and navigation",
      "Handles larger action spaces"
    ],
    "cons": [
  ...
  },
  {
    ...
    "when": "users benefit from strong suggestions while typing and often know the item they want",
    "pros": [
      "Reduces typing",
      "Guides query formation",
      "Speeds up known-item lookup"
    ],
    "cons": [
      "Needs high-quality suggestion data",
      "Less useful for open-ended exploration"
    ]
  },
  {
    "name": "Text Field",
    "path": "/patterns/forms/text-field",
    "when": "the input is freeform text but is not actually performing search or retrieval",
    "pros": [
      "Lowest complexity",
      "Clear form-field affordance"
    ],
    "cons": [
      "No search-specific semantics",
      "No built-in affordance for clear, recent, or result states"
    ]
  }
]}
/>
...
<Playground patternType="forms" pattern="search-field" example="basic" presentation="hidden-code" />
...
<FaqStructuredData
  items={[
  {
    "question": "When should I choose Search Field instead of Autocomplete?",
    "answer": "Choose search field when the job depends on help users retrieve content or commands quickly with a clear input, sensible defaults, and immediate recovery paths. If the team only needs a lighter interaction with fewer states, Autocomplete will usually be easier to ship and maintain."
  },
  {
    "question": "What is the biggest implementation risk with Search Field?",
  ...
  {
    "question": "How do I know whether search field is working well?",
...
<RelatedPatternsCard
  patterns={[
    ...
    },
    {
      ...
      path: "/patterns/advanced/search-results",
      description: "Display and filter search results",
    },
  ]}
/>

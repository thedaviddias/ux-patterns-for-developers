...
<BuildEffort
  level="medium"
  description="Requires a multi-step flow: email request form, token generation and email delivery, token validation on the reset page, and new password form with validation. Secure token management and rate limiting add backend complexity."
/>
  ...
  alternatives={[
    {
      ...
      when: "the user remembers their password and needs to authenticate",
      pros: ["Single step", "No email required"],
      cons: ["Useless if password is forgotten"]
    },
    {
      name: "Social Login",
      path: "/patterns/authentication/social-login",
      when: "the user can authenticate via a third-party provider instead",
      pros: ["No password to remember", "Instant access"],
      cons: ["Only works if social account is linked", "Provider dependency"]
    },
    {
      name: "Two-Factor Authentication",
      path: "/patterns/authentication/two-factor",
      when: "additional verification is needed during the reset process",
      pros: ["Stronger identity verification", "Protects against email compromise"],
      cons: ["Added complexity", "Requires prior 2FA setup"]
    }
  ]}
/>
...
<Playground patternType="authentication" pattern="password-reset" example="basic" presentation="hidden-code" />
...
- **Timing:
import { Playground } from "@/components/playground";** Real-time on each keystroke in the confirm field
...
<FaqStructuredData
  items={[
    {
      question: "How long should a password reset link be valid?",
      answer:
        "Password reset links should expire within 1 hour, with 15-60 minutes being the recommended range. Shorter expiry times are more secure but may frustrate users who check email infrequently. Always provide a way to request a new link when one expires.",
    },
    {
      question: "Should I tell users if their email isn't in the system?",
      answer:
        "No. Always show the same confirmation message regardless of whether the email exists. This prevents account enumeration attacks where attackers test email addresses against your system to find valid accounts.",
    },
    {
      question: "What happens to existing sessions after a password reset?",
      answer:
        "All existing sessions should be invalidated after a successful password reset. This ensures that if the reset was triggered due to a compromised account, the attacker's sessions are also terminated.",
    },
    {
      question: "How do I prevent password reset email abuse?",
      answer:
        "Implement rate limiting: restrict to 3-5 reset requests per email per hour. Use consistent response timing to prevent timing attacks. Consider adding an invisible CAPTCHA after repeated requests from the same IP.",
    },
    {
      question: "Should the password reset form require the old password?",
      answer:
        "No. The password reset flow is for users who have forgotten their password. The reset token serves as identity verification. However, for logged-in users changing their password in account settings, requiring the current password is recommended.",
    },
  ]}
/>

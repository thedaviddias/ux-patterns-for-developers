...
<BuildEffort
  level="medium"
  description="Requires form validation, secure credential handling, error messaging, rate limiting considerations, and accessibility for password fields. Optional features like remember me, social login, and passwordless flows add complexity."
/>
  ...
  alternatives={[
    {
      ...
      when: "users prefer authenticating via existing accounts (Google, GitHub, Apple)",
      pros: ["No password to remember", "Faster sign-in", "Verified email"],
      cons: ["Provider dependency", "Privacy concerns", "Limited to provider availability"]
    },
    {
      name: "Sign Up Flow",
      path: "/patterns/authentication/signup",
      when: "the user doesn't have an account yet and needs to register",
      pros: ["Creates new accounts", "Collects user data", "Onboarding opportunity"],
      cons: ["Higher friction", "More fields", "Verification required"]
    },
    {
      name: "Two-Factor Authentication",
      path: "/patterns/authentication/two-factor",
      when: "additional security is needed beyond username and password",
      pros: ["Stronger security", "Protects against stolen passwords"],
      cons: ["Extra step", "Device dependency", "Recovery complexity"]
    }
  ]}
/>
...
<Playground patternType="authentication" pattern="login" example="basic" presentation="hidden-code" />
...
import { Playground } from "@/components/playground";, forcing users to retype their email address.
...
<FaqStructuredData
  items={[
    {
      question: "What is a login form?",
      answer:
        "A login form is a user interface pattern that allows returning users to authenticate and access protected content. It typically includes an email or username field, a password field, a submit button, and links to password recovery and registration.",
    },
    {
      question: "How do I make a login form accessible?",
      answer:
        "Use native HTML form elements with proper labels, associate errors with fields via aria-describedby, mark invalid fields with aria-invalid, provide a password visibility toggle with accurate aria-label, and ensure the form is fully operable via keyboard.",
    },
    {
      question: "Should login forms show specific error messages?",
      answer:
        "No. Avoid messages that reveal whether an email exists in the system (like 'Wrong password' or 'No account found'). Use a generic message such as 'Invalid email or password' to prevent account enumeration attacks.",
    },
    {
      question: "What is passwordless login?",
      answer:
        "Passwordless login authenticates users without a password, using methods like magic email links, SMS one-time codes, biometric authentication (fingerprint, face), or passkeys. It reduces friction and eliminates password-related security risks.",
    },
    {
      question: "Should I include social login on the login page?",
      answer:
        "Social login reduces friction for users with existing accounts on providers like Google, GitHub, or Apple. Include it when your audience uses these providers and your application doesn't require email-based identity. Always offer an alternative for users without social accounts.",
    },
  ]}
/>

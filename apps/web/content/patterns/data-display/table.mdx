...
icon: Table
ogImage: /covers/patterns/table.png
status: complete
aliases: ["data grids"]
popularity: high
bestFor:
  - Dense records where side-by-side comparison is the job
  - Data that benefits from sorting and column scanning
  - Operational interfaces like admin, analytics, and inventory
avoidWhen:
  - Mobile reading or visual browsing is the main use case
  - Each item needs strong imagery or self-contained context
  - Users mostly scan one record at a time
compareWith:
  - name: "List View"
...
<BuildEffort
level="high"
  description="Complex component requiring sorting, filtering, [pagination](/glossary/pagination), responsive design, and accessibility considerations. May need virtual scrolling for large datasets."
/>
<GuidesBanner

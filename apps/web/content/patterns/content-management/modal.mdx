...
icon: Maximize2
ogImage: /covers/patterns/modal.png
status: complete
aliases: ["dialog", "dialog box", "overlay", "popup"]
popularity: high
bestFor:
  - Critical confirmations and blocking actions
  - Focused forms or short multi-step tasks
  - Content that must temporarily take over attention
avoidWhen:
  - The action is lightweight or purely contextual
  - Users still need the surrounding page while interacting
  - The content is long enough to deserve its own route
compareWith:
  - name: "Popover"
...
<PatternComparison
  current="Modal"
  title="Choose the right overlay pattern"
  description="Modal is the strongest interruptive option. Compare it with lighter contextual surfaces before taking over the whole screen."
  alternatives={[
    {
      name: "Popover",
      path: "/patterns/content-management/popover",
      when: "the content belongs to a trigger and should feel contextual rather than interruptive",
      pros: ["Keeps page context", "Lighter interaction cost", "Good for quick actions"],
      cons: ["Easier to dismiss accidentally", "Less space and focus control"]
    },
    {
      name: "Tooltip",
      path: "/patterns/content-management/tooltip",
      when: "users only need a brief explanation and there is nothing meaningful to click inside",
      pros: ["Minimal UI cost", "Fast to scan", "Great for labels and hints"],
      cons: ["No real interaction", "Weak fit for critical or longer content"]
    }
  ]}
/>
...
items={[
    ...
    },
    {
      ...
      answer:
        "To ensure accessibility, use [semantic HTML](/glossary/semantic-html) elements like `<dialog>` or `<div>` with appropriate ARIA roles. Manage focus by moving it to the modal when it opens and returning it to the triggering element upon closure. Provide keyboard support for navigation and actions within the modal.",
    },
    {

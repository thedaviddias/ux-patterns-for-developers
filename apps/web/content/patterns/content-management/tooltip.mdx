...
icon: MessageCircle
status: complete
aliases: ["hover hint", "info bubble"]
---
...
<BuildEffort
level="low"
  description="Simple hover/focus effect. Requires [ARIA attributes](/glossary/aria-attributes) (`aria-describedby`) for accessibility but no complex state management."
/>
<GuidesBanner
  title="Tooltip vs Popover vs Modal?"
...
<PatternComparison
  ...
  description="Tooltip only works when the content stays brief and non-critical. If it needs more than that, step up to a richer surface."
  alternatives={[
    {
      name: "Popover",
      path: "/patterns/content-management/popover",
      when: "the content is still contextual but needs richer text, controls, or lightweight interaction",
      pros: ["Supports more content", "Can handle actions", "Still keeps page context"],
      cons: ["Heavier than a tooltip", "Needs stronger dismissal and focus handling"]
    },
    {
      name: "Modal",
      path: "/patterns/content-management/modal",
      when: "the information is important enough to interrupt the flow or the user must act before continuing",
      pros: ["Maximum focus", "Best for required tasks", "Clearer for important decisions"],
      cons: ["Most disruptive option", "Overkill for small explanations"]
    }
  ]}
/>
...
<Playground patternType="content-management" pattern="tooltip" example="basic" height="600px" />

...
export const PatternWrapper = ({ pattern }: PatternWrapperProps) => {
  ...
  return (
    <div className={wrapperClasses}>
      {pattern.status !== 'coming-soon' ? (
        <LinkCustom href={pattern.href} className="!no-underline w-full" icon={false}>
          {content}
        </LinkCustom>
      ) : (
        content
...
const PatternContent = ({ pattern }: PatternWrapperProps) => (
  ...
  >
    ...
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between">
        <h3 className="font-display text-xl font-semibold text-foreground">{pattern.title}</h3>
      </div>
      <p className="mb-3 text-sm text-foreground leading-6!">{pattern.summary}</p>
    </div>
  </div>
);

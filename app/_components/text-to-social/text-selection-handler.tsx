...
import { trackTextToSocialEvent } from '@app/_utils/tracking';
...
function SelectionPopover({ selectedText, position, onClose }: SelectionPopoverProps) {
  ...
  const generateImage = async (platform: 'instagram' | 'twitter' | 'linkedin') => {
    ...
    try {
      ...
      const response = await fetch('/api/social-image', {
        ...
        body: JSON.stringify({ text: selectedText, platform, patternName: pageTitle, pageUrl }),
      });
    ...
    if (text.length > 10 && text.length < 500) {
      ...
      const walkNodes = (node: Node) => {
        if (node.nodeType === Node.TEXT_NODE) {
          ...
          const fontWeight = fwRaw === 'bold' ? 700 : parseInt(fwRaw as string, 10) || 400;

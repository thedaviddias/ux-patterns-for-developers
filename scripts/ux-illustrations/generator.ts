...
async function main() {
  ...
  const targetPattern = targetPatternArg ? targetPatternArg.split('=')[1] : null;
  const targetPairArg = args.find(arg => arg.startsWith('--pair=')); 
  const targetPair = targetPairArg ? targetPairArg.split('=')[1] : null;

  if (!process.env['GEMINI_API_KEY']) {
    console.error('Error: GEMINI_API_KEY environment variable is not set in .env');
    process.exit(1);
  }
  ...
  const promptsFile = path.join(process.cwd(), 'scripts', 'ux-illustrations', 'prompts.json');
  ...
  for (const file of patternFiles) {
    ...
    for (const key of Object.keys(promptsConfig[patternName])) {
      ...
      if (match) {
        ...
        if (!promptsConfig[patternName][newKey] || promptsConfig[patternName][key].locked) {
    ...
    for (const subSection of subSections) {
      ...
      const dos = subSection.match(/\*\*Do's ✅\*\*\s*\n([\s\S]*?)(?=\n\s*\*\*Don'ts ❌\*\*|$)/)?.[1]
        ?.split('\n').map(l => l.trim()).filter(l => l.startsWith('- ')).map(l => l.slice(2)) || [];
      const donts = subSection.match(/\*\*Don'ts ❌\*\*\s*\n([\s\S]*?)(?=\n\s*\*\*|$)/)?.[1]
        ?.split('\n').map(l => l.trim()).filter(l => l.startsWith('- ')).map(l => l.slice(2)) || [];
        ...
        if (numPairs > 1) {
          try {
            ...
            for (let i = 0; i < numPairs; i++) {
              pairsList.push(`${i}: DO: ${dos[i] || 'N/A'} | DONT: ${donts[i] || 'N/A'}`);
            }
...
Return ONLY the index number of the best pair.`;
            ...
            console.warn(`  Failed AI selection for "${subTitle}", defaulting to first pair.`);
          }
        }

        const selectedDo = dos[bestPairIndex] || dos[0] || `Correct implementation of ${subTitle}`;
        const selectedDont = donts[bestPairIndex] || donts[0] || `Wrong implementation of ${subTitle}`;
...
AESTHETIC: Apple/Dribbble minimalist product design. High visual balance and alignment.`
      ...
      await writeFile(promptsFile, JSON.stringify(promptsConfig, null, 2), 'utf8');

      if (isScanOnly) continue;

      if (entry.locked && !isForceAll) {
        console.log(`  Skipping ${pairSlug}: Locked in prompts.json (use --force-all to override)`);
        continue;
      }

      const outDir = path.join(process.cwd(), `apps/web/public/patterns/${patternName}`);
      ...
      try {
        ...
        if (imageBuffer) {
          ...
          const optimizedBuffer = await sharp(imageBuffer).webp({ quality: 80, effort: 6 }).toBuffer();
          await writeFile(path.join(outDir, `${pairSlug}.webp`), optimizedBuffer);

...
const REGISTRY_JSON_PATH = path.join(PROJECT_ROOT, "packages/registry/registry.json")
const OUTPUT_PATH = path.join(PROJECT_ROOT, "__registry__")
...
async function buildRegistryIndex() {
  ...
  const blockComponents = registry.items.filter(item =>
...
export const Index: Record<string, any> = {`
  ...
  for (const component of blockComponents) {
    ...
    const componentPath = `../packages/registry/registry/default/blocks/${componentName}`
  ...
  console.log(`✅ Registry index built with ${blockComponents.length} components`)
}

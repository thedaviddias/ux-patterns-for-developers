...
import type { SuggestPatternResponse } from "../types";

export const suggestPatternDefinition = {
	name: "suggest_pattern",
	description:
		"Get pattern suggestions based on your current development context",
	inputSchema: {
		...
		properties: {
			context: {
				type: "string",
				description:
					'Description of what you are building (e.g., "user signup form with validation")',
			},
			category: {
				type: "string",
...
const patternKeywords: Record<string, string[]> = {
	...
	"content-management/popover": ["popover", "dropdown", "menu", "context"],
	"content-management/tooltip": ["tooltip", "hint", "help", "info", "hover"],
	"content/cards": ["card", "cards", "grid", "item", "preview"],
	"data-display/card-grid": ["card", "cards", "grid", "item", "preview"],
	"content/table": ["table", "data", "grid", "rows", "columns", "list"],
	"data-display/table": ["table", "data", "grid", "rows", "columns", "list"],
	"data-display/comparison-table": [
		"compare",
		"comparison",
		"pricing",
		"table",

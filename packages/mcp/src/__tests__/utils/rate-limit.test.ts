...
describe("Rate Limit", () => {
	...
	describe("checkRateLimit", () => {
		...
		it("should return rate limit headers", () => {
			...
			expect(result.headers).toHaveProperty("X-RateLimit-Limit", "30");
		...
		it("should track different identifiers separately", () => {
			...
			expect(result1.remaining).toBe(26); // 4th request
			expect(result2.remaining).toBe(29); // 1st request
		});
		...
		it("should start new window after reset time passes", () => {
			...
			const result2 = checkRateLimit(identifier);
			expect(result2.remaining).toBe(29); // Back to 29
		...
		it("should allow requests again after window resets", () => {
			...
			expect(allowedResult.remaining).toBe(29);
		});
		...
		it("should provide reset time in seconds in headers", () => {
			...
			const resetHeaderValue = parseInt(
